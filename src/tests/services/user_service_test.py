...
def get_user_service(
    settings: Settings, bot_mock: mock.Mock, repo_store_mock: RepoStoreMock
):
    return UserService(repo_store=repo_store_mock, settings=settings, bot=bot_mock)
...
@pytest.mark.parametrize(
    "user_from_db",
    [(UserEntity(id=1, telegram_id=1, name="NoneName", email="email"),), (None,)],
)
async def test_user_service_get_or_none(
    user_from_db: UserEntity | None,
    user_service: UserService,
    repo_store_mock: RepoStoreMock,
):
