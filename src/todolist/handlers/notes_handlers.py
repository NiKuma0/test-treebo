...
async def addnote_entrypoint_handler(
    _: Message, answer_to: AnswerToAbc, state: FSMContext, user: UserEntity | None
):
    if user is None:
    ...
    await answer_to("Send me text of the note:")
...
async def set_note_text(message: Message, answer_to: AnswerToAbc, state: FSMContext):
    ...
    await answer_to(
        f"Send date and time in format {datetime.datetime.now().strftime(DATETIME_FORMAT)}"
    )


@router.message(UserInputStates.note_datetime)
async def addnote_endpoint_handler(
    message: Message,
    ...
    try:
        ...
        await answer_to(
            f"Can't parse date and time - value ({message.text}) doesn't match format."
        )
        return
    ...
    try:
        ...
        await answer_to(
            "Notifications into the past are not possible. Timezone should be "
        )
        return
