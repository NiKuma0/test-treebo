...
async def set_email_handler(message: Message, state: FSMContext, answer_to: AnswerToAbc, services_store: ServicesStore, event_from_user: User):
    if message.text is None: return
    data = await state.get_data()
    await services_store.user_service.register(email=message.text, name=data['name'], telegram_id=event_from_user.id)
    await state.clear()
    await answer_to(
